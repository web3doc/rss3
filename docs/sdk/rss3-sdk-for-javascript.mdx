...
支持的帐户列表可在 [API#Supported account](/guide/api.html#supported-account)
...
1. 将我们的主要地址或指向我们主要地址的名称（参见 [API#Supported name service](/guide/api.html#supported-name-service)）添加到 Twitter bio、name 或 url
...
支持的自动资产列表可在 [API#Supported auto assets](/guide/api.html#supported-auto-assets) 获得
...
[API#Supported auto items](/guide/api.html#supported-auto-items) 中提供了字段的可能值。
